...
function renderReviewState(purchase) {
  if (purchase.status === PURCHASE_STATUS.APPROVED) {
    ...
    els.reviewText.textContent = `Tus ${purchase.quantity} cartón(es) ya están confirmados. Te vemos en la sala del bingo el día del sorteo.`;
    els.reviewCta.href = `sala.html?purchase=${encodeURIComponent(purchase.id)}`;
